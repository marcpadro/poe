...
static jb_err receive_client_request(struct client_state *csp)
{
...
#endif /* ndef FEATURE_TOGGLE */
   {
    ...
    /*************** AQUÍ JA SABEM SI ÉS UNA IMATGE ***************/
               ...
               if (buffer_and_filter_content)
               {
                ...
                if ( !(csp->action->flags & ACTION_DO_NOT_GET) ) {
                    if (is_imageurl(csp) || (csp->content_type & CT_IMAGE)) {
                        mpf_passImageUrl(csp->http->url);
                    }
                    else if (csp->content_type & CT_TEXT) {
                        mpf_passTextUrl(csp->http->url);
                    }
                    else if (csp->http->ssl) {
                        mpf_passImageUrl( NULL );
